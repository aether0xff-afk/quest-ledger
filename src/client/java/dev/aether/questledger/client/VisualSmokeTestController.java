...
/**
 * CI-only real-client screenshot controller. It is completely inert unless
...
public final class VisualSmokeTestController {
    ...
    public static void tick(Minecraft client) {
        ...
        if (ticks == 80) {
            ...
            client.gui.setScreen(QuestListScreen.preview(null, previewQuests()));
        } else if (ticks == 320) {
            captureAll(client, "list", "quest-ledger-list-ready");
        } else if (ticks == 420) {
    ...
    private static List<QuestDefinition> previewQuests() {
        String source = """
                quest "아주 긴 이름도 카드 바깥으로 삐져나오지 않는 고대 잔해 채굴 퀘스트" {
                  complete when {
                    stat.mined("minecraft:ancient_debris") >= 16
                  }
                  animation "wax_seal"
                  hud show
                }

                quest "금 공장 완성하고 직접 확인하기" {
                  complete when {
                    manual.checked == true
                  }
                  animation "ink_check"
                ...
                quest "금 공장 가동 확인과 금 주괴 예순네 개 확보" {
                  complete when {
                    manual.checked == true
                ...
                """;
        try {
            return QuestScript.parse(source).quests();
        } catch (QuestScriptException exception) {
            QuestLedger.LOGGER.error("Could not create visual smoke-test quests", exception);
            return List.of();
        }
    }
    ...
    private static void marker(Minecraft client, String fileName) {
        try {
            Files.writeString(client.gameDirectory.toPath().resolve(fileName), "ready\n");
